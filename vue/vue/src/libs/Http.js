...
export default {
    ...
    ajax(requestParam) {
        ...
        var finallyCallback = function (data) {
            if (requestParam.finally) {
                requestParam.finally(extract(data, 'finally'))
            }
        }
...
function buildUrlV14(rp) {
    // let serverUrl = getV2ServerUrl()
    let serverUrl = "http://192.168.32.13:8080/zx"
    ...
    return appendParams(cleanPath(serverUrl + '/' + rp.methodName), rp.params)
}